...
type
  TFrmMain = class(TForm)
    ...
    DBGrid1: TDBGrid;
    Button3: TButton;
    EstacasProjetosperimetro: TFloatField;
    EstacasProjetosid_estacas_tipos: TIntegerField;
  ...
  public
    ...
    procedure _Resultante(start: Boolean;descSolo:string;profcamada:string;spt:string;sptTxt:string);
    procedure _MostraCalculo;
  end;
  const scala=20;
  ...
  for cota1:=1 to StrToInt(FormatFloat('#####0',StrToFloat(pCol_1))) do begin
    with myDraw1 do begin
      ...
      if (sSoloChk2<>sSoloChk1) then begin
        ...
        _Resultante(false,sSoloChk1,IntToStr(sSoloQtd),IntToStr(sSptVlr),sSptTxt );
        sSptVlr:=0;
        sSptTxt:='';
        sSoloQtd:=0;
        sSoloChk1:=sSoloChk2;
      ...
      //Canvas.TextOut(480+sCol, lastProf+12,IntToStr(lastProf)+' >>> teste');
      if (iSpt>0) then begin
       sSptVlr:=sSptVlr+iSpt;
       sSptTxt:=sSptTxt+IntToStr( _GetSPT( cota1 ) )+'+';
      end;
...
procedure TFrmMain._Resultante(start: Boolean;descSolo:string;profcamada:string;spt:string;sptTxt:string);
var
  sSql:string;
  idSolo,idProj,idMetodo,fatorSolo,fatorEstacaF1,fatorEstacaF2:string;
  sDiv,sKn,R1:Real;
  sEstaca,sDiametro,sPerimetro:string;
  sTxt,sTxt1:string;

  _calc_R1:Real;
  _calc_R1_Txt:String;
  sIdEstacaTipo:String ;
  tot_Kn:Real;
begin
 idProj:=Trimright(Projetosid.AsString);
 if idProj='' then Exit;
 if start then begin
   sSql:='DELETE FROM projetos_calculos WHERE id_projetos='+idProj;
   _ExecSql(sSQL);
 end;

 idMetodo:=Trimright(QryMetodosid.AsString);
 if idMetodo='' then Exit;


 descSolo:=TrimRight(TrimLeft( descSolo ));
 if descSolo='' then Exit;
 sSql:='SELECT id FROM solos WHERE descricao=' +#39+descSolo+#39;
 idSolo:=_ReturnField(sSql);



 sSql:='SELECT vlr_alpha FROM metodos_solos WHERE id_metodos='+idMetodo+' AND id_solos='+idSolo;
 fatorSolo:=_ReturnField(sSQL);
 if fatorSolo='' then fatorSolo:='0';



 if ( Copy(sptTxt,Length(sptTxt),1)='+' ) then begin
  sptTxt:=Copy(sptTxt,1,Length(sptTxt)-1);
 end;
 sDiv:=0;
 if (StrToFloat( profCamada)>0) then begin
  sDiv:=  StrToFloat( spt ) /  StrToFloat( profcamada );
 end;

 sTxt:='Spt Medio: ('+sptTxt+')/'+profcamada+'='+_VirgulaParaPonto( FormatFloat('####0.00',sDiv));

 sKn:= ((sDiv/3)+1)*10;

 sTxt:=sTxt+' r1 = [ ('+ _VirgulaParaPonto( FormatFloat('####0.00',sDiv))+'/3)+1] x 10 ='+FloatToStr(sKn)+' Kn/m2';

 with EstacasProjetos do begin
    if not Active then Exit;
    First;
    while not eof do begin
       sDiametro :=Fieldbyname('diametro').AsString;
       sPerimetro:=FieldByName('perimetro').AsString;
       sEstaca   :=Fieldbyname('id_estacas_diametros').AsString;
       sIdEstacaTipo   :=Fieldbyname('id_estacas_tipos').AsString;

       sSql:='select fator_f1 from estacas_diametros where id_estacas_tipos='+sIdEstacaTipo+' and diametro='+sDiametro;
       fatorEstacaF1:=_ReturnField(sSQL);
       if fatorEstacaF1='' then fatorEstacaF1:='0';

       sSql:='select fator_f2 from estacas_diametros where id_estacas_tipos='+sIdEstacaTipo+' and diametro='+sDiametro;
       fatorEstacaF2:=_ReturnField(sSQL);
       if fatorEstacaF2='' then fatorEstacaF2:='0';


       sDiametro :=FloatToStr( StrToFloat( sDiametro )/100 );
       sPerimetro:=FloatToStr( StrToFloat( sPerimetro )/100 );
       R1:= StrToFloat(sPerimetro) * StrToFloat( profCamada) * sKn * StrToFloat(fatorEstacaF1);
       sTxt1:=sTxt+',  R1 = ' + sPerimetro +'(Perimetro)x'+ profCamada+'(Prof.)x'+FloatToStr( sKn )+'(KN/m2)x'+fatorEstacaF1+' (Fator F1 para '+Fieldbyname('descricao').AsString+') ='+FloatToStr(R1);


       _calc_R1:= sKn;
       _calc_R1_Txt :=    ' r1 = [ ('+ _VirgulaParaPonto( FormatFloat('####0.00',sDiv))+'/3)+1] x 10 ='+FloatToStr(sKn)+' Kn/m2';

       tot_kn:=R1;

       sSql:='INSERT INTO projetos_calculos ('+
             ' id_projetos,'+
             ' id_solos,'+
             ' id_estacas_diametros,'+
             ' id_metodos,'+
             ...
             #39+idMetodo+#39+','+
             #39+profcamada+#39+','+
             #39+_VirgulaParaPonto(fatorSolo)+#39 +','+
             #39+_VirgulaParaPonto(spt)+#39 + ','+
             #39+sTxt1+#39+ ','+
             #39+_VirgulaParaPonto( FloatToStr(_calc_R1 ))+#39 + ','+
             #39+_calc_R1_Txt+#39+ ','+
             #39+_VirgulaParaPonto( FloatToStr(tot_kn ))+#39 + 
             ')';
       _ExecSql(sSQL);
...
procedure TFrmMain._MostraCalculo;
var
  sSql:String;
  iProj:String;
  _Qry: TZQuery;
  iLine:Integer;
  idDiam,descDiam:String;
  iPosLine,iPosCol:Integer;
  sPosStr:Integer;
  sStr,sStr1,sStr2,sStr3:String;
  tot_Kn:Real;
begin
  iProj:=TrimRight(projetosid.AsString);
  if iProj='' then exit;
  ...
  with myDraw1 do begin
    ...
    Canvas.Font.Size:=8;
     ...
     iPosLine:=100;
     iPosCol:=850;
     Canvas.TextOut(iPosCol, iPosLine,'BASE DE CALCULO');
     Canvas.MoveTo(iPosCol, iPosLine+20);
     Canvas.LineTo(iPosCol+100, iPosLine+20);
     iPosLine:=iPosLine+30;
    ...
    with EstacasProjetos do begin
      ...
      while not eof do begin
        iPosLine:=iPosLine+30;
        idDiam:= Fieldbyname('id_estacas_diametros').asstring;
        descDiam:=Fieldbyname('descricao').AsString +'  - Diametro:'+Fieldbyname('diametro').asstring;
        Canvas.Font.Color:=_Cor_da_Letra4;
        Canvas.TextOut(iPosCol, iPosLine,descDiam);
        iPosLine:=iPosLine+20;
        Canvas.TextOut(iPosCol, iPosLine,'Solo');
        ...
        sSQL:='SELECT    '+
              ...
              ' s.descricao as solo,  '+
              ' pc.fator_solo,        '+
              ' et.descricao as estaca,'+
              ' ed.diametro as diametro,'+
              ' (ed.perimetro/10) as perimetro,'+
              ' pc.profcamada,'+
              ' pc.spt_medio1,'+
              ' pc.spt_medio_calculo,'+
              ' pc.tot_kn '+
              ' FROM '+
              ' projetos_calculos pc '+
              ' INNER JOIN solos s ON s.id=pc.id_solos '+
              ' INNER JOIN estacas_diametros ed ON ed.id=pc.id_estacas_diametros '+
              ' INNER JOIN estacas_tipos et ON et.id=ed.id_estacas_tipos '+
              ' INNER JOIN metodos m ON m.id=pc.id_metodos '+
              ' WHERE '+
              '  pc.id_projetos ='+ iProj+
              '  AND  pc.id_estacas_diametros='+ idDiam+
              ' ORDER BY '+
              '  pc.id_projetos,pc.id,pc.profcamada ';
              with _Qry do begin
                close;
                Sql.Clear;
                Sql.Add( sSQL ) ;
                Open;
                While not eof do begin
                  Canvas.Font.Color:=_Cor_da_Letra3;
                  Canvas.TextOut(iPosCol, iPosLine,Copy(fieldbyname('solo').AsString,1,15));

                  tot_Kn:=tot_Kn+StrToFloat(fieldbyname('tot_kn').AsString);

                  iPosLine:=iPosLine+10;

                  sStr:=fieldbyname('spt_medio_calculo').AsString;
                  sPosStr:=Pos('R1 =', sStr );

                  sStr1:=Copy(sStr,1,sPosStr-1);
                  sStr2:=Copy(sStr,sPosStr,1000);
                  Canvas.TextOut(iPosCol, iPosLine,sStr1);
                  iPosLine:=iPosLine+15;

                  Canvas.TextOut(iPosCol, iPosLine,sStr2);

                  Canvas.MoveTo(iPosCol, iPosLine+15);
                  Canvas.LineTo(iPosCol+500, iPosLine+15);

                  iPosLine:=iPosLine+30;

                  next;
                end;
                  Canvas.TextOut(iPosCol+450, iPosLine-10,FormatFloat('####0.00',tot_Kn));
                Close;
              end;
